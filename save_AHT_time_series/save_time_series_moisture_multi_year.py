...
for which_year in years:
    ...
    eddy_all_times = sorted_ds.eddy_moist_int.mean(['longitude'], skipna=True).sum(['level'], skipna=True).values
    mmc_all_times = sorted_ds.mmc_moist_int.sum(['level'], skipna=True).values

    np.save('../aht_time_series/moist_aht/mmc_moist_all_times_' + which_year, mmc_all_times)
    np.save('../aht_time_series/moist_aht/eddy_moist_all_times_' + which_year, eddy_all_times)